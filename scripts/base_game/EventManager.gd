...
func setup(p_players: Array, p_turn_manager: TurnManager, p_movement_manager: MovementManager, p_label: Label, pc_label: Label, p_game_manager = null):
	players = p_players
	...
	for p in players:
		...
		if jailed.size() > 0:
			...
			if turn_manager.current_state == TurnManager.State.DUEL:
					# Si hubo duelo, ya se manejará solo
					pass
		elif active.size() > 0:
...
func _show_event_label(text: String):
	...
	print("_show_event_label llamado, game_manager:", game_manager, " is_server:", multiplayer.is_server())
	if game_manager and multiplayer.is_server():
		game_manager.sync_event_label.rpc(text)
